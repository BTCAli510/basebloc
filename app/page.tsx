...
export default function Home() {
  const [activePage, setActivePage] = useState<'home' | 'event'>('home');
    ...
    <>
            ...
            <div style={{maxWidth:1100,margin:'0 auto'}}>
              <div className="carousel" style={{display:'flex',gap:20,overflowX:'auto',scrollSnapType:'x mandatory',WebkitOverflowScrolling:'touch',paddingBottom:16}}>
                ...
                <div
                  ...
                  onClick={() => { setActivePage('event'); window.scrollTo(0,0); }}
                >
                  ...
                  <div style={{padding:16}}>
                    ...
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                      ...
                      <button
                        ...
                        onClick={e => { e.stopPropagation(); setActivePage('event'); window.scrollTo(0,0); }}
                      >View Event</button>
                    </div>
                  </div>
                </div>
      ...
      {activePage === 'event' && (
        <div style={{maxWidth:680,margin:'0 auto',padding:'40px 24px 80px'}}>
          <button
            className="back-btn"
            style={{display:'inline-flex',alignItems:'center',gap:8,fontSize:13,color:'#888888',cursor:'pointer',marginBottom:32,border:'none',background:'none',fontFamily:'DM Sans,sans-serif',transition:'color 0.2s'}}
            onClick={() => { setActivePage('home'); window.scrollTo(0,0); }}
          >← Back to BASE bloc</button>

          <div style={{fontSize:11,fontWeight:600,letterSpacing:2,textTransform:'uppercase',color:'#0052FF',marginBottom:10}}>Hip Hop TV &amp; Citiesabc Present</div>
          <h1 className="syne" style={{fontSize:'clamp(32px,5vw,52px)',fontWeight:800,letterSpacing:'-2px',lineHeight:1.05,marginBottom:12}}>My City Our Music</h1>
          ...
          <button
            style={{width:'100%',background:'#0052FF',color:'#fff',border:'none',borderRadius:12,padding:18,fontFamily:'Syne,sans-serif',fontSize:17,fontWeight:700,cursor:'pointer',marginBottom:12,letterSpacing:'-0.5px',transition:'background 0.2s'}}
            onClick={() => showToast('Opening RSVP in Base App...')}
          >RSVP on Base</button>
          <div style={{textAlign:'center',fontSize:12,color:'#888888',marginBottom:32}}>Opens in the Base App · Powered onchain by BASE bloc · Gas sponsored</div>

          <div style={{marginBottom:32}}>
            <h3 className="syne" style={{fontSize:20,fontWeight:700,letterSpacing:'-0.5px',marginBottom:10}}>About the summit</h3>
            <p style={{fontSize:15,lineHeight:1.7,color:'#888888'}}>My City Our Music is a one-day summit at the intersection of music, creative industries, and artificial intelligence — produced by Hip Hop TV and Citiesabc at the iconic Henry J. Kaiser Center for the Arts in Oakland.</p>
            <p style={{fontSize:15,lineHeight:1.7,color:'#888888',marginTop:12}}>This summit brings together artists, technologists, and community leaders to explore how culture shapes technology — and how technology can serve culture. Oakland is the first stop.</p>
          </div>
          ...
          <div style={{background:'#F7F7F7',border:'1px solid #EFEFEF',borderRadius:16,padding:28}}>
            ...
            <p style={{fontSize:14,lineHeight:1.7,color:'#888888'}}>Your RSVP creates a verified onchain record that shows you were part of this moment. As BASE bloc grows, that record can help power community recognition, future access, and opportunities for those who show up.</p>
            {[
              ...
              <div key={r.title} style={{display:'flex',alignItems:'flex-start',gap:12,marginTop:16}}>
                ...
                <div style={{fontSize:13,lineHeight:1.6,color:'#444444'}}><strong style={{color:'#0A0A0A'}}>{r.title}</strong> {r.desc}</div>
              </div>
            ))}
          </div>

          <div style={{textAlign:'center',fontSize:13,color:'#0052FF',fontWeight:600,marginTop:40,fontFamily:'Syne,sans-serif',letterSpacing:'0.3px'}}>Power to the People. Onchain.</div>
        </div>
      )}
